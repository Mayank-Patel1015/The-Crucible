...
MISSIONS = {
    1: {
        ...
        "prompt": "From first principles, implement a 1-neuron perceptron in pure Python that computes weighted sum + sigmoid. Explain every single symbol as if teaching a 12-year-old. No libraries, no copy-paste."
    }
}

def run_llama(prompt):
    system_prompt = """<|system|>You are a strict Socratic crucible. Point out ONLY logical faults, missing definitions, or unstated assumptions in the user's attempt. NEVER give code, answers, or hints. Ask ONE sharp question that forces deeper human generation. Under 80 words.<|end|>
<|user|>Topic: """ + prompt + """<|end|>
<|assistant|>"""
    cmd = [LLAMA_CLI, "-m", MODEL, "-p", system_prompt, "--temp", "0.1", "-n", "100", "--no-mmap"]
...
def record_attempt(day, topic, human, ai, joules, secs):
    ...
    conn.execute("""INSERT INTO socratic_sessions 
        (mission_day, topic, human_attempt, ai_response, human_word_count, ai_word_count, cqr, energy_joules, duration_seconds)
        VALUES (?,?,?,?,?,?,?,?,?)""",
        (day, topic, human, ai, metrics["breakdown"]["human_words"], metrics["breakdown"]["ai_words"], metrics["cqr"], joules, secs))
    conn.commit()
    conn.close()
    return metrics["cqr"]
...
def plot_progress():
    conn = sqlite3.connect(DB)
    rows = conn.execute("SELECT mission_day, cqr FROM socratic_sessions ORDER BY mission_day").fetchall()
    conn.close()
    if rows:
